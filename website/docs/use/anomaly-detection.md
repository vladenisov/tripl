...
A scope holding an outage open is checked against the series it stopped producing, and a scope that never produced anything stores no rows to check — its head stays frozen at the report, so the "still collecting" test stays true for it forever. Reports of that shape (`0 actual` against `0 expected`, which a project running `min_expected_count = 0` can accumulate) were therefore permanently open, and were the one shape nothing could ever close. What that changes is the **denominator** on the Anomalies page: the *N* of "M of N open" no longer counts rows nobody could act on. The **Significant** view, the sidebar badge and the Overview headline do not move, because a zero-versus-zero row has no magnitude and never passed the gate those surfaces apply.
:::
