...
export function OnboardingReturnBar({ className = '' }: { className?: string }) {
  ...
  return (
    <nav
      ...
      className={`flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded-card border px-3 py-2 ${className}`}
    ...
    >
      ...
      <span className="min-w-0 flex-1 truncate text-body-sm">
        <span style={{ color: 'var(--fg-subtle)' }}>Getting started ·</span>{' '}
        <span className="font-medium">{target.title}</span>
      </span>
      <Link
        to={projectHomePath(target.slug)}
        className="flex shrink-0 items-center gap-1 text-caption font-medium no-underline hover:underline"
