...
/**
 * On the header row: only the pinned cells (select-all and "Event") stay; the
 * column headers and their filters have no column to sit over in a card.
 */
export const PHONE_HEADER_ROW =
  'max-md:flex max-md:items-center max-md:[&>th]:static! max-md:[&>th:not(.tripl-pin-l)]:hidden'
 ...
 * Δ 24h, last seen, owner, custom field and meta values).
 */
export const PHONE_DROPPED_CELL = 'max-md:hidden'

/** A full-width row that is not an event (loading, empty state). */
export const PHONE_FULL_ROW = 'max-md:block max-md:[&>td]:block'
