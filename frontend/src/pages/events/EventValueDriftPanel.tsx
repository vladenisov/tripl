...
export function EventValueDriftPanel({ slug, eventId }: { slug: string; eventId: string }) {
  ...
  const activeDrifts = (data?.items ?? []).filter(
  ...
  const actionMut = useMutation({
    mutationFn: ({ driftId, action, scope, snoozedUntil }: {
      driftId: string
      action: 'accept' | 'snooze' | 'false_positive'
      scope?: 'global' | 'event'
      snoozedUntil?: string
              ...
              <div className="min-w-0">
                <div className="font-mono text-xs font-medium">
                  ...
                  {drift.status === 'snoozed' && (
                    <span className="ml-1.5 rounded border px-1 py-0.5 text-[10px] font-normal text-muted-foreground">snoozed</span>
                  )}
                </div>
