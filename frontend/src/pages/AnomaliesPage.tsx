...
export default function AnomaliesPage() {
  ...
  for (const signal of byMagnitude) {
    countsAtLevel.set(signal.scan_config_id, (countsAtLevel.get(signal.scan_config_id) ?? 0) + 1)
  }
  const scanTotals = new Map<string, number>()
  for (const signal of signals) {
    scanTotals.set(signal.scan_config_id, (scanTotals.get(signal.scan_config_id) ?? 0) + 1)
  }
  ...
  const filtered =
    activeScanId === ALL_SCANS
      ? byMagnitude
      : byMagnitude.filter((s) => s.scan_config_id === activeScanId)
  ...
  const activeScanLabel = scanNames.get(activeScanId) ?? 'this scan'
