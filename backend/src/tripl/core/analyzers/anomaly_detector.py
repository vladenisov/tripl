...
def _collapse_outage_runs(
    anomalies: list[DetectedAnomaly], expanded: list[SeriesPoint]
) -> list[DetectedAnomaly]:
    """Keep only the FIRST bucket of each run of buckets that emitted nothing.
    ...
    0 at 72-96h and 28 again at 120-168h, most of them at z=-20.00 — the exact
    z the 5% relative stddev floor pins a drop-to-zero at. The non-monotonicity is the
    ...
    below it (and, on a hard death, overshoots negative) — so suppression that
    hangs off the live trend level switches itself off exactly when the outage
    gets older.

    Anchoring on the run's first bucket instead makes the claim independent of
    the fit: the incident is announced by the row at the bucket where the scope
    went silent, and because ``_replace_scope_anomalies`` only rewrites rows
    INSIDE the evaluation window, that row stays on record after the onset
    scrolls out of the window. Later scans of the same outage re-derive the same
    anchor, find it outside their window, and write nothing — the persisted row
    is the announcement, not the current trend level.

    Runs are contiguous in the ANALYZED series, not on the grid: a bucket the
    ...
    for a fractional series 0.0 is a value (a ratio or level that happens to be
    zero, and possibly not even its minimum), so "the same incident" would need a
    different definition there.
    """
    run_start_bucket: dict[datetime, datetime] = {}
    ...
    for point in expanded:
        ...
        if run_start is None:
            run_start = point.bucket
        run_start_bucket[point.bucket] = run_start
    return [
        anomaly
        for anomaly in anomalies
        if run_start_bucket.get(anomaly.bucket, anomaly.bucket) == anomaly.bucket
    ]
...
def detect_anomalies(
    ...
    *,
    ...
    A scope that has gone silent is announced ONCE, at the bucket where it went
    silent, and stays quiet until it emits again — see ``_collapse_outage_runs``.
    ...
    if not is_count_shaped:
        return merged
    return _collapse_outage_runs(merged, expanded)
